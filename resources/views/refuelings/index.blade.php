<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <a href="{{ route('refuelings.create', ['car_id' => $carId ?? '']) }}" 
                    ...
                    <form method="GET" action="{{ route('refuelings.index') }}" class="space-y-4">
                        <div class="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4">
                            ...
                            <div class="flex gap-2 items-end">
                                ...
                                <a href="{{ route('refuelings.index') }}" class="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition px-3 py-2.5">
                                    Сбросить
                                </a>
                            </div>
                        </div>
            ...
            @if($refuelings->isEmpty())
                <div class="bg-white dark:bg-[#222222] overflow-hidden shadow-sm rounded-xl">
                    <div class="p-12 text-center">
                        <p class="text-gray-500 dark:text-gray-400 font-medium mb-2">Нет данных о заправках</p>
                        <p class="text-sm text-gray-400 dark:text-gray-500 mb-6">Добавьте первую заправку для вашего автомобиля</p>
                        <a href="{{ route('refuelings.create', ['car_id' => $carId ?? '']) }}" 
                        ...
                        <table class="min-w-full">
                            <thead>
                                <tr class="bg-gray-50 dark:bg-[#1a1a1a] border-b border-gray-100 dark:border-gray-700">
                                    ...
                                    <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Литры</th>
                                    <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Цена/л</th>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">АЗС</th>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                @foreach($refuelings as $refueling)
                                    <tr class="hover:bg-[#E5E7EB] dark:hover:bg-[#1D1D1D] transition-all duration-200">
                                        ...
                                        <td class="px-4 py-3 text-sm font-semibold text-red-600 dark:text-red-400 text-right whitespace-nowrap">
                                        ...
                                        <td class="px-4 py-3 text-right whitespace-nowrap">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ route('refuelings.show', $refueling) }}" 
                    ...
                    @foreach($refuelings as $refueling)
                        <div class="bg-white dark:bg-[#222222] overflow-hidden shadow-sm rounded-xl p-4">
                            ...
                            <div class="grid grid-cols-2 gap-3 mb-3 p-2 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                                <div>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Литры</p>
                                    <p class="text-sm font-semibold text-gray-800 dark:text-white">
                                        {{ number_format($refueling->converted_liters, 2) }} {{ $refueling->volume_unit }}
                                    </p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Цена за литр</p>
                                    <p class="text-sm font-semibold text-gray-800 dark:text-white">
                                        {{ number_format($refueling->converted_price, 2) }} {{ $refueling->currency }}
                                    </p>
                                </div>
                                ...
                                <div>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">АЗС</p>
                            ...
                            <div class="flex gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                                ...
                                <form action="{{ route('refuelings.destroy', $refueling) }}" method="POST" onsubmit="return confirm('Вы уверены, что хотите удалить эту заправку?')" class="flex-1">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <div class="px-4 sm:px-6 py-4">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <div class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 text-center sm:text-left">
                                    Показано с {{ $refuelings->firstItem() }} по {{ $refuelings->lastItem() }} 
                                    из {{ $refuelings->total() }} заправок
                                </div>
                                
                                <div class="flex items-center justify-center gap-1 overflow-x-auto pb-1 sm:pb-0">
                                    @if($refuelings->onFirstPage())
                                    ...
                                    @else
                                        <a href="{{ $refuelings->previousPageUrl() }}" class="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition whitespace-nowrap">Назад</a>
                                    @endif
                                    
                                    <div class="flex gap-0.5 sm:gap-1">
                                        @foreach($refuelings->getUrlRange(1, $refuelings->lastPage()) as $page => $url)
                                            @php $currentPage = $refuelings->currentPage(); $lastPage = $refuelings->lastPage(); @endphp
                                            @if($page == $currentPage)
                                    ...
                                    @if($refuelings->hasMorePages())
                                        <a href="{{ $refuelings->nextPageUrl() }}" class="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition whitespace-nowrap">Вперед</a>
                                    @else
