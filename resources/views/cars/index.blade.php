        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-[#222222] overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    @if($cars->isEmpty())
                        <p class="text-gray-500 dark:text-gray-400 text-center py-8">У вас пока нет добавленных автомобилей. Нажмите кнопку выше, чтобы добавить.</p>
                    @else
                            ...
                            <table class="min-w-full table-auto">
                                <thead>
                                    <tr class="bg-gray-100 dark:bg-[#6B727F]">
                                        ...
                                        <th class="px-4 py-2 text-left w-40">Действия</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($cars as $car)
                                        <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-[#E5E7EB] dark:hover:bg-[#1D1D1D] transition">
                                            ...
                                            <td class="px-4 py-2 text-right">
                                                <div class="flex justify-end gap-2">
                                                    <a href="{{ route('cars.edit', $car) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded text-sm transition">
                                                        Редактировать
                                                    </a>
                                                    <form action="{{ route('cars.destroy', $car) }}" method="POST" onsubmit="return confirm('Вы уверены, что хотите удалить этот автомобиль?')">
                                                        @csrf
                                                        @method('DELETE')
