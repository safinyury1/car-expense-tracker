<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <a href="{{ route('expenses.create', ['car_id' => $carId ?? '']) }}" 
                    ...
                    <form method="GET" action="{{ route('expenses.index') }}" class="space-y-4">
                        <div class="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4">
                            ...
                            <div class="flex-1 min-w-[160px]">
                                ...
                                <div class="relative">
                                    <select name="category_id" class="w-full border-gray-300 dark:border-gray-600 dark:bg-[#4B5563] dark:text-white rounded-lg shadow-sm text-sm pl-3 pr-8 py-2.5 appearance-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            <div class="flex-1 min-w-[180px]">
                                ...
                                <input type="text" name="search" value="{{ $search ?? '' }}" 
                                       placeholder="Описание, категория, марка..."
                            ...
                            <div class="flex gap-2 items-end">
                                ...
                                <a href="{{ route('expenses.index') }}" class="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition px-3 py-2.5">
                                    Сбросить
                                </a>
                            </div>
                        </div>
            ...
            @if($expenses->isEmpty())
                <div class="bg-white dark:bg-[#222222] overflow-hidden shadow-sm rounded-xl">
                    <div class="p-12 text-center">
                        <p class="text-gray-500 dark:text-gray-400 font-medium mb-2">Нет данных о расходах</p>
                        <p class="text-sm text-gray-400 dark:text-gray-500 mb-6">Добавьте первый расход для вашего автомобиля</p>
                        <a href="{{ route('expenses.create', ['car_id' => $carId ?? '']) }}" 
                        ...
                        <table class="min-w-full">
                            <thead>
                                <tr class="bg-gray-50 dark:bg-[#1a1a1a] border-b border-gray-100 dark:border-gray-700">
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Описание</th>
                                ...
                                @foreach($expenses as $expense)
                                    <tr class="hover:bg-[#E5E7EB] dark:hover:bg-[#1D1D1D] transition-all duration-200">
                                        ...
                                        <td class="px-4 py-3 text-sm font-semibold text-red-600 dark:text-red-400 text-right whitespace-nowrap">
                                            {{ number_format($expense->converted_amount, 2) }} {{ $expense->currency }}
                                        </td>
                                        <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300 text-right whitespace-nowrap">
                                            {{ number_format($expense->converted_odometer) }} {{ $expense->distance_unit }}
                                        </td>
                    ...
                    @foreach($expenses as $expense)
                        <div class="bg-white dark:bg-[#222222] overflow-hidden shadow-sm rounded-xl p-4">
                            ...
                            <div class="flex gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                                ...
                                <form action="{{ route('expenses.destroy', $expense) }}" method="POST" onsubmit="return confirm('Вы уверены, что хотите удалить этот расход?')" class="flex-1">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <div class="px-4 sm:px-6 py-4">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <div class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 text-center sm:text-left">
                                    Показано с {{ $expenses->firstItem() }} по {{ $expenses->lastItem() }} 
                                    из {{ $expenses->total() }} расходов
                                </div>
                                ...
                                <div class="flex items-center justify-center gap-1 overflow-x-auto pb-1 sm:pb-0">
                                    ...
                                    @else
                                        <a href="{{ $expenses->previousPageUrl() }}" class="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition whitespace-nowrap">Назад</a>
                                    @endif
                                    
                                    <div class="flex gap-0.5 sm:gap-1">
                                        @foreach($expenses->getUrlRange(1, $expenses->lastPage()) as $page => $url)
                                            @php $currentPage = $expenses->currentPage(); $lastPage = $expenses->lastPage(); @endphp
                                            @if($page == $currentPage)
                                    ...
                                    @if($expenses->hasMorePages())
                                        <a href="{{ $expenses->nextPageUrl() }}" class="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition whitespace-nowrap">Вперед</a>
                                    @else
