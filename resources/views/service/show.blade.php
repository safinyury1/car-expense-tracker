<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <a href="{{ route('service.index', ['car_id' => $service->car_id]) }}" 
                ...
                <div class="p-5 sm:p-7 md:p-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4">
                                <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">Тип обслуживания</p>
                                <p class="text-lg font-semibold text-gray-800 dark:text-white mt-1">
                                    {{ $service->title }}
                                </p>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4">
                                ...
                                <p class="text-lg font-semibold text-gray-800 dark:text-white mt-1">
                                    {{ number_format($service->converted_odometer) }} {{ $service->distance_unit }}
                                </p>
                            </div>
                            <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4">
                                <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">Стоимость</p>
                                <p class="text-lg font-semibold text-red-600 dark:text-red-400 mt-1">
                                    {{ number_format($service->converted_cost, 2) }} {{ $service->currency }}
                                </p>
                            </div>
                        </div>
                        ...
                        <!-- Следующее ТО -->
                        @if($service->next_due_odometer || $service->next_due_date)
                            <div class="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border border-blue-200 dark:border-blue-800">
                                <p class="text-xs text-blue-600 dark:text-blue-400 uppercase tracking-wider">Следующее ТО</p>
                                <div class="flex flex-wrap gap-4 mt-1">
                                    @if($service->next_due_odometer)
                                        <p class="text-sm font-medium text-blue-700 dark:text-blue-300">
                                            Пробег: {{ number_format($service->next_due_odometer) }} км
                                        </p>
                                    @endif
                                    @if($service->next_due_date)
                                        <p class="text-sm font-medium text-blue-700 dark:text-blue-300">
                                            Дата: {{ \Carbon\Carbon::parse($service->next_due_date)->format('d.m.Y') }}
                                        </p>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="pt-4 border-t border-gray-100 dark:border-gray-700">
                            <div class="flex justify-between items-center mb-3">
                                <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                    Прикреплённые файлы ({{ $service->attachments->count() }}/4)
                                </h4>
                                @if($service->attachments->count() < 4)
                            ...
                            <form id="addAttachmentForm" action="{{ route('service.add-attachment', $service) }}" 
                            ...
                            @if($service->attachments && $service->attachments->count() > 0)
                    ...
                    <div class="flex justify-between items-center mt-8 pt-4 border-t border-gray-100 dark:border-gray-700">
                        <a href="{{ route('overview.index', ['car_id' => $service->car_id]) }}" 
                           class="bg-gray-500 hover:bg-gray-600 text-white font-medium py-2 px-5 rounded-lg text-sm transition">
                            Назад
                        </a>
                        <form action="{{ route('service.destroy', $service) }}" method="POST" onsubmit="return confirm('Вы уверены, что хотите удалить это обслуживание?')" class="inline">
                            @csrf
                            @method('DELETE')
    ...
    <script>
        ...
        document.getElementById('addAttachmentInput').addEventListener('change', function(e) {
            ...
            const currentCount = {{ $service->attachments->count() }};
            const maxFiles = 4;
